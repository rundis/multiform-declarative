...
(function () {
    ...
    sfk.saveableForm = _.extend(bane.createEventEmitter(), {
        create: function (params) {
            ...
            var bindChangeToSaveButtonToggle = function(component) {
                if(component.on) {
                    component.on("change", function() {
                        var compValid = function(comp) {return !comp.isValid || comp.isValid(); };
                        var formValid = c.all(compValid, components);
                        if(formValid) {
                            saveButton.enable();
                        } else {
                          saveButton.disable();
            ...
            c.doall(bindChangeToSaveButtonToggle, components);
